...
y   <- dnorm(x,mean=(4/9), sd=sqrt(14/(81*32)))
...
ks.test(data_gp32$hypothetical_pct,"pnorm",mean=4/9,sd=sqrt(14/(81*32)))

#subset 2003,2004, 2007-2010 all gp=30
data_gp30<-subset(data,year==2003 | year==2004 | year>=2007 & year<=2010)

x   <- seq(0,1,length=1000)
y   <- dnorm(x,mean=(4/9), sd=sqrt(14/(81*30)))
...
#kolmogorov-smirnov test that dist are diff
ks.test(data_gp30$pct,"pnorm",mean=4/9,sd=sqrt(14/(81*30)))
...
x   <- seq(0,1,length=1000)
y   <- dnorm(x,mean=(4/9), sd=sqrt(14/(81*34)))
...
#kolmogorov-smirnov test that dist are diff
ks.test(data_gp34$pct,"pnorm",mean=4/9,sd=sqrt(14/(81*34)))
